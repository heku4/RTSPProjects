...
public class RtspRequest : RtspMessage
{
    ...
    public RtspRequest(string rawRequestData)
    {
        ...
        if (mainRequestData.Length != 3) throw new Exception($"Main request data is not valid: '{rows[0]}'");

        var methodParsed = Enum.TryParse<RtspMethod>(mainRequestData[0], out var method);
        if (!methodParsed) throw new Exception($"Can't parse request method: '{rows[0]}'");
        ...
        foreach (var row in rows.Skip(1))
        {
            ...
            if (header.Length != 2) throw new Exception($"Can't parse request header: '{row}'");
            // continue;
            if (header[0] == "CSeq")
            {
                var seqParsed = int.TryParse(header[1], out var number);
                if (!seqParsed) throw new Exception($"Can't parse CSeq number: '{row}'");
